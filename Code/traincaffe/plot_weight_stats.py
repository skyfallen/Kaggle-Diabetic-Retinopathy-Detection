...
if len(sys.argv) != 5:
    ...
    print 'Usage example: python explorenetwork.py EMPTY size256 onehidden 2015-06-10-01-35'
    sys.exit(2)

# cmd arguments and env variables
...
def plotstats(data, layer_names, ylabel, preprocessing_type, prefix, model_name, date, filename):
    plt.clf()
    ...
    plt.savefig('/home/' + user + '/Kaggle/Diabetic-Retinopathy-Detection/Code/traincaffe/networks/' + preprocessing_type + '/' + prefix + model_name + '/plots/' + date + '/' + filename)
    plt.show()

# plot weights ratio
plotstats(net_stats[:, 0:len(layer_names)], layer_names, 'ratio of weights', preprocessing_type, prefix, model_name, date, 'weight_ratios.png')

# plot mean of weights
plotstats(net_stats[:, len(layer_names):2*len(layer_names)], layer_names, 'mean of weights', preprocessing_type, prefix, model_name, date, 'mean_weights.png')

# standard deviation of weights
plotstats(net_stats[:, 2*len(layer_names):3*len(layer_names)], layer_names, 'stds of weights', preprocessing_type, prefix, model_name, date, 'stds_weights.png')
