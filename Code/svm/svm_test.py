...
print "Loading model..."
with open('/storage/hpc_anna/Kaggle_DRD/svm/b10size256/svm_1_0.0001.pkl', 'r') as f:
	clf = cPickle.load(f)
...
with open('/storage/hpc_anna/Kaggle_DRD/predictions/b10size256/' + date + '_predictions_svm_test.csv', 'w') as f:
        f.write('image,level\n')
        for fid, file in enumerate(test_files):
                f.write(file + ',' + str(test_predictions[fid]) + '\n')
print 'Submission file saved as' + '/storage/hpc_anna/Kaggle_DRD/predictions/b10size256/' + date + '_predictions_svm_test.csv'
